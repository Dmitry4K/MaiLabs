...
void castString(std::string& str) {
	char tmpChar = ' ';
	int stringLen;
	if (BLOCK_ID == 0) {
...
struct camera_settings {
	...
	void cast() {
		...
		MPI_Bcast(&angle, 1, MPI_FLOAT, 0, MPI_COMM_WORLD);
	}
} CAMERA_SETTINGS;
...
public:
	...
	void cast() {
		...
		MPI_Bcast(&A_r, 1, MPI_FLOAT, 0, MPI_COMM_WORLD);
		MPI_Bcast(&A_z, 1, MPI_FLOAT, 0, MPI_COMM_WORLD);
		MPI_Bcast(&omega_r, 1, MPI_FLOAT, 0, MPI_COMM_WORLD);
		MPI_Bcast(&omega_z, 1, MPI_FLOAT, 0, MPI_COMM_WORLD);
		MPI_Bcast(&omega_phi, 1, MPI_FLOAT, 0, MPI_COMM_WORLD);
		MPI_Bcast(&p_r, 1, MPI_FLOAT, 0, MPI_COMM_WORLD);
		MPI_Bcast(&p_z, 1, MPI_FLOAT, 0, MPI_COMM_WORLD);
...
void castFloat3(float3& f) {
	MPI_Bcast(&f.x, 1, MPI_FLOAT, 0, MPI_COMM_WORLD);
	MPI_Bcast(&f.y, 1, MPI_FLOAT, 0, MPI_COMM_WORLD);
	MPI_Bcast(&f.z, 1, MPI_FLOAT, 0, MPI_COMM_WORLD);
}
...
struct base_figure {
	...
	void cast() {
		...
		MPI_Bcast(&reflection, 1, MPI_FLOAT, 0, MPI_COMM_WORLD);
		MPI_Bcast(&transparency, 1, MPI_FLOAT, 0, MPI_COMM_WORLD);
		MPI_Bcast(&countOfLights, 1, MPI_INT, 0, MPI_COMM_WORLD);
		MPI_Bcast(&isLight, 1, MPI_C_BOOL, 0, MPI_COMM_WORLD);
	}
};
...
void input() {
	...
	castString(OUTPUT_RENDERED_IMAGE_PATH);
	CAMERA_SETTINGS.cast();
	CAMERA_MOVEMENT_SETTINGS.cast();
	FIGURE_1.cast();
	FIGURE_2.cast();
	FIGURE_3.cast();
	FLOOR.cast();
	MPI_Bcast(&LIGHTS_COUNT, 1, MPI_INT, 0, MPI_COMM_WORLD);
	...
	MPI_Barrier(MPI_COMM_WORLD);
	MPI_Bcast(&RECURSIVE_DEPTH, 1, MPI_INT, 0, MPI_COMM_WORLD);
	MPI_Bcast(&SQRT_OF_LIGHTS_COUNT_PER_PIXEL, 1, MPI_INT, 0, MPI_COMM_WORLD);
}
...
void render_cpu(float3 pc, float3 pv, int w, int h, float angle, uchar4* data, int frame_num) {
	...
	#pragma omp parallel
	{
		...
		for (int it = threadId; it < w * h; it += threadCount) {
			int i = it % w,j = it / w;
			int pixel_num = i * h + j;
...
__global__ void render_gpu(
	...
	int threadId, int threadTotalCount
) {
...
void cpu_ssaa(uchar4* picture, const uchar4* image, int w, int h, int sqrt_per_pixel) {
	...
	#pragma omp parallel
	{
		...
		int threadId = omp_get_thread_num();
		int threadCount = omp_get_num_threads();
		for (int it = threadId; it < h *w; it += threadCount) {
			int i = it % h, j = it / h;
			//for (int i = 0; i < h; ++i) {
			//	for (int j = 0; j < w; ++j) {
...
void cpu_calc() {
	...
	for (int frame_number = BLOCK_ID; frame_number < FRAMES_COUNT; frame_number += BLOCKS_COUNT) {
...
void gpu_calc() {
	...
	for (int frame_number = BLOCK_ID; frame_number < FRAMES_COUNT; frame_number+=BLOCKS_COUNT) {
		...
		{
			int threadId = omp_get_thread_num();
			int totalThreadCount = omp_get_num_threads();
		...
		sprintf(buff, OUTPUT_RENDERED_IMAGE_PATH.c_str(), frame_number);
		std::cout << "PROC:" << BLOCK_ID << "[" << frame_number + 1 << '/' << FRAMES_COUNT << "]: " << buff << std::endl;
...
__host__ void add_icosahedron(base_figure figure) // ���������� ������ ������
{
	// ��������� ��������
	float R = figure.radius;
	float a = 4 * R / sqrtf(10 + 2 * sqrtf(5)); // ������� ���������
	float alpha = acosf((1 - a * a / 2 / R / R)); // ������ ���� �������� �� ����
	float k = PI / 180.0f;
	// ����� ���������
	std::vector<float3> ver_mas;
...
int main(int argc, char* argv[]) {
	int device_count;
	CEH(cudaGetDeviceCount(&device_count));
	CEH(cudaSetDevice(BLOCK_ID % device_count));
	std::ios::sync_with_stdio(false);
	key_config key_cfg(argc, argv);
	MPI_Init(&argc, &argv);
	MPI_Comm_size(MPI_COMM_WORLD, &BLOCKS_COUNT);
